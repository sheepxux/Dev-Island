...
/// height was a fixed 360pt guess a full task list pushed the "Connect
/// Service" footer outside the clip and it simply vanished.
final class PanelGeometryTests: XCTestCase {
    ...
    /// top of the current hardware range), the 8pt gap it puts above the
    /// list, the 1pt footer divider, and the ~37pt Connect Service row.
    private let chromeBudget: CGFloat = 40 + 8 + 1 + 37
    ...
    func testPanelCeilingFitsAFullTaskListPlusChrome() {
        XCTAssertGreaterThanOrEqual(
            ...
            "Raising the task-list cap or adding a footer row also needs a taller panelMaxHeight, or the panel clips its own footer."
        )
    }
