...
actor TunnelManager {
    ...
    func stop() async {
        ...
        await cloudflaredProcess?.stop()  // [C→S] CloudflaredProcess is an actor
    ...
    private func checkProcessHealth() async {
        // [C→S] proc.isRunning is actor-isolated; needs await.
        guard let proc = cloudflaredProcess else { return }
        let isRunning = await proc.isRunning
